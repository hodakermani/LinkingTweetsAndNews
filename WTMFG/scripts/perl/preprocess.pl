...
die "Usage: perl preprocess.pl input_file output_file" if (@ARGV != 2);
...
### 1. tokenize
my $cmd;
$cmd = "perl $home_dir/tokenize_en.pl $prefix $prefix.tok";
print "\n[Step 1]: $cmd\n";
`$cmd`;
sleep(1);
...
### 2. stemming
$cmd = "cat $prefix.tok | perl $home_dir/stemmer.pl > $prefix.stem";
print "\n[Step 2]: $cmd\n";
`$cmd`;
sleep(1);
...
### 3. clean the words
$cmd = "perl $home_dir/clean.pl  $prefix.stem  $prefix.clean";
print "\n[Step 3]: $cmd\n";
`$cmd`;
sleep(1);
