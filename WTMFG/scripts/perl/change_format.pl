...
my $cmd;
if ($is_train == 1) {
    $cmd = "perl $home_dir/rm_infreq.pl $train_file $train_file.rmiw 2";
    print "$cmd\n";
    `$cmd`;
    #$train_file .= ".rmiw";
    $file .= ".rmiw";
...
### 2. index words
$cmd = "perl $home_dir/index.pl $is_train $file $file.ind $vocab_file";
print "$cmd\n";
`$cmd`;
...
$cmd = "perl $home_dir/tfidf.pl $is_train $file $idf_file $tf $idf $norm";
print "$cmd\n";
`$cmd`;
if ($clean == 1) {
    $cmd = "rm $file";
...
$cmd = "perl $home_dir/matlab_format.pl $file $file.ml";
